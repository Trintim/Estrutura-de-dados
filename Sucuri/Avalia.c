        ...
        if (front(posFixa)->categoria == INT || front(posFixa)->categoria == FLOAT){
            ...
            switch (front(posFixa)->categoria){
                ...
                case OPER_EXPONENCIACAO:;
                    ...
                    Objeto *valor1 = copiaObjeto(topoPilha(entrada));
                    desempilha(entrada);
                    Objeto *valor2 = copiaObjeto(topoPilha(entrada));
                    desempilha(entrada);
                    if (valor1->categoria == FLOAT){
                        if(valor2->categoria == FLOAT){
                            valor2->valor.vFloat = pow(valor2->valor.vFloat, valor1->valor.vFloat);
                        }
                        else{
                            valor2->valor.vFloat = pow(valor2->valor.vInt, valor1->valor.vFloat);
                            valor2->categoria = FLOAT;
                        }
                    }
                    else if(valor2->categoria == FLOAT){
                        valor2->valor.vFloat = pow(valor2->valor.vFloat, valor1->valor.vInt);
                    }
                    else{
                        valor2->valor.vInt = pow(valor2->valor.vInt, valor1->valor.vInt);
                    }

                    empilha(entrada, copiaObjeto(valor2));
                    liberaObjeto(valor2);
                    liberaObjeto(valor1);
                    break;
